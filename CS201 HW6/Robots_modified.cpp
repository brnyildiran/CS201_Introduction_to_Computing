...
/**************** START: CS201 students' member functions ********************/ 
...
//Stalled --> True --> Dead || Dead --> IsAlive --> False 
//Stalled --> False --> Alive || Alive --> IsAlive --> True
//stalled and IsAlive are opposites of each other.
bool Robot :: IsAlive()
{
	return(!stalled);
}

//Mutator function --> changing the private variable
void Robot :: TurnFace(Direction dir)
{
	if(!stalled) //If robot is not dead
	{
		direction = dir;
		//Do this whenever something visually changes
		theRobotWindow->Redraw(this); //Redraw the Robot World
	}
}
...
/**************** END: CS201 students' member functions ********************/ 
